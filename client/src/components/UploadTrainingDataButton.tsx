      ...
      for (let index = 0; index < trainingEndpoints.length; index += 1) {
        ...
        try {
          ...
          if (!response.ok || !payload?.success) {
            const errorMessage =
              payload?.message ||
              `Не удалось обновить модель (${response.status}).`;
          ...
          lastError =
            attemptError instanceof Error
              ? attemptError
              : new Error("Не удалось обновить модель.");

          console.warn(
            `[ML][train-upload] Ошибка при обращении к ${endpoint}:`,
            lastError,
          );
        }
      }
  ...
  return (
    <Card className="bg-card/80 backdrop-blur-sm border-primary/20 shadow-md">
      <CardHeader>
        ...
        <CardDescription>
          Отправьте Excel-файл с ежедневной выручкой (минимум 3 месяца), чтобы переобучить ML-модель.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {(startedAt || completedAt) && (
          <div className="rounded-md border border-primary/10 bg-muted/30 p-3 text-xs text-muted-foreground">
            {startedAt && (
              <p>
                Начато: {startedAt.toLocaleString("ru-RU")}
              </p>
            )}
            {completedAt && (
              <p>
                Завершено: {completedAt.toLocaleString("ru-RU")}
              </p>
            )}
          </div>
        )}
