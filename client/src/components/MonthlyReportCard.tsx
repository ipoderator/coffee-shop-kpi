...
export function MonthlyReportCard({ monthlyData }: MonthlyReportCardProps) {
  ...
  const avgRevenue =
    recentMonths.reduce((sum, month) => sum + month.revenue, 0) / recentMonths.length;
  const avgChecks =
    recentMonths.reduce((sum, month) => sum + month.checks, 0) / recentMonths.length;
  const avgCheckAmount =
    recentMonths.reduce((sum, month) => sum + month.averageCheck, 0) / recentMonths.length;
  ...
  const variance =
    recentMonths.reduce((sum, month) => sum + Math.pow(month.revenue - avgRevenue, 2), 0) /
    recentMonths.length;
  ...
  const getVolatilityLevel = (volatility: number) => {
    if (volatility < 10)
      return { level: 'Низкая', color: 'text-green-600', bgColor: 'bg-green-50' };
    if (volatility < 20)
      return { level: 'Умеренная', color: 'text-yellow-600', bgColor: 'bg-yellow-50' };
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="space-y-2">
            ...
            <p className="text-xl font-bold">
              {avgRevenue.toLocaleString('ru-RU', { maximumFractionDigits: 0 })} ₽
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xl font-bold">
              {avgChecks.toLocaleString('ru-RU', { maximumFractionDigits: 0 })}
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xl font-bold">
              {avgCheckAmount.toLocaleString('ru-RU', { maximumFractionDigits: 0 })} ₽
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p
              className={`text-xl font-bold ${trendPercentage >= 0 ? 'text-green-600' : 'text-red-600'}`}
            >
              {trendPercentage >= 0 ? '+' : ''}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-3">
            ...
            <Progress value={Math.min(volatilityPercentage, 50)} className="h-2" />
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Badge
                variant={
                  volatilityPercentage < 15
                    ? 'default'
                    : volatilityPercentage < 25
                      ? 'secondary'
                      : 'destructive'
                }
              >
            ...
            <Progress value={Math.max(0, 100 - volatilityPercentage)} className="h-2" />
            <p className="text-xs text-muted-foreground">Чем выше, тем стабильнее бизнес</p>
          </div>
        </div>
                ...
                >
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {month.checks} чеков • средний чек{' '}
                      {month.averageCheck.toLocaleString('ru-RU', { maximumFractionDigits: 0 })} ₽
                    </p>
                  </div>
