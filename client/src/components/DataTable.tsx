...
type SortField = 'date' | 'amount' | 'dailyAvgCheck' | 'deviation';
...
export function DataTable({ transactions }: DataTableProps) {
  ...
  const transactionsWithMetrics = useMemo((): TransactionWithMetrics[] => {
    ...
    dailyMap.forEach((data, day) => {
      dailyAvgChecks.set(day, data.revenue / data.checks);
    });
    ...
    monthlyMap.forEach((data, month) => {
      monthlyAvgChecks.set(month, data.revenue / data.checks);
    });
    ...
    return transactions.map(t => {
      ...
      const deviation = monthlyAvgCheck > 0 
        ? ((dailyAvgCheck - monthlyAvgCheck) / monthlyAvgCheck) * 100 
        : 0;
          ...
          <tbody>
            {sortedTransactions.map((transaction, index) => (
              ...
              >
                ...
                <td className="p-4 text-sm font-mono text-right tabular-nums" data-testid={`cell-amount-${index}`}>
                ...
                <td className="p-4 text-sm font-mono text-right tabular-nums" data-testid={`cell-daily-avg-check-${index}`}>
                  {new Intl.NumberFormat('ru-RU', {
                    style: 'currency',
                    currency: 'RUB',
                    minimumFractionDigits: 0,
