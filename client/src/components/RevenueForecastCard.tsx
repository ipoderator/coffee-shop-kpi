...
export function RevenueForecastCard({ forecast }: RevenueForecastCardProps) {
  ...
  const calculateExternalDataQuality = (forecast: RevenueForecast) => {
    ...
    if (methodology.holidayAnalysis) qualityScore += 0.1;
    if (methodology.trendAnalysis) qualityScore += 0.1;
    if (methodology.seasonalAdjustment) qualityScore += 0.1;
    ...
    const dataPointsBonus = Math.min(0.2, methodology.dataPoints / 1000); // До 20% за 1000+ точек
  ...
  const activeDailyForecast =
    forecastView === 'nextMonth' ? nextMonth.dailyForecast : extendedForecast.dailyForecast;
  ...
  const totalDays =
    activeDailyForecast.length > 0
      ? activeDailyForecast.length
      : Math.max(methodology.forecastDays, 1);
  ...
  const isGeneratedWeek = (week: GeneratedWeek | WeeklyForecast): week is GeneratedWeek =>
    'week' in week;
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>
          Многомерное прогнозирование с учетом региональных особенностей кофейного бизнеса, включая
          нестабильность выходных дней, расширенный ансамбль ML-методов, анализ временных рядов и
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="text-center p-4 bg-blue-50 rounded-lg">
            ...
            <div className="text-xs text-blue-500 mt-1">
              ±
              {Math.round(calculateForecastRange(summaryPredictedRevenue, summaryConfidence).range)}
          ...
          <div className="text-center p-4 bg-green-50 rounded-lg">
            <div className={`text-2xl font-bold ${getConfidenceColor(summaryConfidence)}`}>
              {Math.round(summaryConfidence * 100)}%
            </div>
            <div className={`text-sm ${getConfidenceColor(summaryConfidence)}`}>
            ...
            <div className="group relative overflow-hidden">
              ...
              <div className="relative p-6 bg-gradient-to-br from-purple-50 via-purple-100 to-purple-200 rounded-2xl border-2 border-purple-300 shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1">
                ...
                <p className="text-sm text-purple-800 font-medium">
                  {methodology.forecastDays} дней вперед
                </p>
                <div className="mt-3 flex items-center gap-2">
                ...
                <div className="relative p-5">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div
                        className={`relative p-3 rounded-xl shadow-md group-hover:shadow-lg transition-all duration-300 group-hover:scale-110 ${
                          methodology.seasonalAdjustment
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-green-700 mt-1">
                    ✨ Высокая точность даже с малым количеством данных
                  </p>
                </div>
        ...
        {(forecastView === 'nextMonth' || forecastPeriod === 'weekly') && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {(forecastView === 'nextMonth'
                ? weeklyForecasts
                : extendedForecast.weeklyForecast
              ).map((week, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Card
                      className={`relative p-6 border-2 shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 hover:scale-105 ${
                        isHighConfidence
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                          <div
                            className={`relative p-2 rounded-xl shadow-md group-hover:shadow-lg transition-all duration-300 group-hover:scale-110 ${
                              isHighConfidence
                                ? 'bg-gradient-to-r from-emerald-500 to-emerald-700'
                                : isMediumConfidence
                                  ? 'bg-gradient-to-r from-blue-500 to-blue-700'
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {(() => {
                  ...
                  for (let i = 1; i < weeks.length; i++) {
                    ...
                    const currentRevenue = isGeneratedWeek(currentWeek)
                      ? currentWeek.revenue
                      : currentWeek.predictedRevenue;
                    const previousRevenue = isGeneratedWeek(previousWeek)
                      ? previousWeek.revenue
                      : previousWeek.predictedRevenue;
                    ...
                    trends.push({
                      week: isGeneratedWeek(currentWeek)
                        ? currentWeek.week
                  ...
                  return trends.map((trend, index) => (
                    <div key={index} className="group relative overflow-hidden">
                      ...
                      <div
                        className={`relative p-4 rounded-xl border-2 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 ${
                          trend.isPositive
              ...
              {extendedForecast.monthlyForecast.map((month, index) => (
                <Card key={index} className="p-4">
                  <div className="text-center">
                    ...
                    <div className="text-sm text-gray-600 mb-2">{month.monthName}</div>
                    ...
                    <div className="text-xs text-gray-500 mt-1">
                      ±
                      {Math.round(
                        calculateForecastRange(month.predictedRevenue, month.confidence).range,
                      )}
                      % • {month.dailyCount} дней
                    </div>
                  </div>
                </Card>
              ))}
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="text-right">
                      <div className="font-semibold text-sm">
                        {formatRange(day.predictedRevenue, day.confidence)}
                      </div>
                      ...
                      <div className="text-xs text-gray-500">
                        ±
                        {Math.round(
                          calculateForecastRange(day.predictedRevenue, day.confidence).range,
                        )}
                        %
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-xs">
                    <div className="flex items-center gap-2">
                      ...
                      {day.weatherImpact !== undefined && (
                        <span
                          className={`flex items-center gap-1 ${getImpactColor(day.weatherImpact)}`}
                        >
                    ...
                    <div className="flex items-center gap-2">
                      {day.factors?.holiday.isHoliday ? (
                        <>
                          ...
                          <span className="text-gray-600">
                            {day.factors.holiday.holidayType === 'national'
                              ? 'Гос. праздник'
                              : day.factors.holiday.holidayType === 'religious'
                                ? 'Религиозный'
                                : day.factors.holiday.holidayType === 'regional'
                                  ? 'Региональный'
                                  : 'Праздник'}
                          </span>
                          {day.holidayImpact !== undefined && (
                            <span
                              className={`flex items-center gap-1 ${getImpactColor(day.holidayImpact)}`}
                            >
        ...
        <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
          <div className="text-sm text-green-800">
            <strong>Повышенная точность прогнозирования:</strong> Система использует расширенную
            интеграцию внешних источников данных для значительного сокращения разброса прогнозов.
            Включает данные о погоде, экономических индикаторах, праздниках, трафике и социальных
            настроениях. Многомерный ансамбль ML-методов с анализом временных рядов, поведенческим
            моделированием и нейронными сетями. Диапазоны прогнозов сокращены за счет качества
            внешних данных: ±3-20% от прогнозируемого значения (сокращение до 50% при высоком
            качестве данных).
          </div>
        </div>
      </CardContent>
